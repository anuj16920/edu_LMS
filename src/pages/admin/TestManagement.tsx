...
const TestManagement = () => {
  ...
  const addQuestion = () => {
    ...
    setFormData({
      ...formData,
      totalQuestions: (questions.length + 1).toString(),
    });
  };
  ...
  const removeQuestion = (index: number) => {
    ...
    setFormData({
      ...formData,
      totalQuestions: newQuestions.length.toString(),
    });
  };
  ...
  const handleUpdateTest = async () => {
    ...
    try {
      ...
      const response = await apiClient.put(`/tests/${editingTest._id}`, testData);
  ...
  const handleToggleActive = async (id: string) => {
    try {
      ...
      console.log("✅ Test toggled:", response.data);
  ...
  return (
    <DashboardLayout role="admin">
      <div className="space-y-6">
        ...
        {/* Edit Dialog - Same as Create but with Update button */}
        <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
          <DialogContent className="bg-card border-border max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="text-2xl font-bold">Edit Test</DialogTitle>
            </DialogHeader>
            {/* Same form as create, but call handleUpdateTest */}
            <div className="space-y-4 mt-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium mb-2 block">Test Title</label>
